...
from AST import *
from main.csel.utils.AST import BooleanType, NumberType, StringType
...
class ASTGeneration(CSELVisitor):
    ...
    def visitId_list_element_const(self, ctx: CSELParser.Id_list_element_constContext):
        ...
        if ctx.composit():
            ...
            dimen = []
        if ctx.Typ():
            typq = self.visit(ctx.Typ().getText())
            if typq == 'Number':
                typ = NumberType()
            elif typq == 'Boolean':
                typ = BooleanType()
    ...
    def visitTyp(self, ctx: CSELParser.Typ):
        if ctx.Number():
            return NumberType()
        elif ctx.Boolean():
            return BooleanType()
    ...
    def visitComposit(self, ctx: CSELParser.CompositContext):
        ...
        lst = []
        for j in intlist:
            i = NumberLiteral(float(j.getText()))
            lst += [i]
    ...
    def visitAssign_stm(self, ctx: CSELParser.Assign_stmContext):
        lhs = []
        rhs = []
        if (ctx.array_cell() | ctx.json_cell()):
            lhs = self.visit(ctx.array_cell() | ctx.json_cell())
        else:
    ...
    def visitCall_exp(self, ctx: CSELParser.Function_callContext):
        id = Id(ctx.ID().getText())
        param = []
        if ctx.exp():
            for i in range(len(ctx.exp())):
